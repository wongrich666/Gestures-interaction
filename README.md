...
为了降低 live 模式卡顿，手部识别已经限频到约 24fps，FaceLandmarker 约 10fps，Canvas 渲染仍跟随 `requestAnimationFrame`，渲染时复用最近一次识别结果。实时模式默认使用 `medium` 摄像头档位，避免一启动就用 720p 给普通 Windows 笔记本带来过高压力。复杂手势会先进入轻量状态机，只有候选手势稳定一小段时间后才切换到 `enter / hold / exit`，减少频繁误触发。进入实时模式会自动请求摄像头；麦克风不可用时不会阻断摄像头和手部粒子。
...
| 实时演出模式 | 在视觉风格中选择 `现实液化` 或 `晶体折射`，摄像头底图会进入 WebGL 液化层 |
...
| WebGL 液化 | 物理模拟半分辨率执行，FBO/texture 只在 resize 时重建 |
